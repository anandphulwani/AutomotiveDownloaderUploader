...
for (let username of allUsernamesFromConfig) {
    ...
    const excelFilename = `${reportGenerationPath}\\${username}_${monthInMMM}_${year}.xlsx`;
    ...
    const dealersLength = dealerNumbers.length;
    ...
    // // Iterate over extractedData and print each row
    // for (let i = 0; i < transposedData.length; i++) {
    //     const row = transposedData[i];
    //     console.log(`Row ${i + 1}:`);
    //     for (let j = 0; j < row.length; j++) {
    //         console.log(`  Column ${j + 1}: ${row[j]}`);
    //     }
    ...
    for (let i = 0; i < transposedData.length; i++) {
        ...
        for (let j = 1; j < transposedData[2].length; j++) {
            ...
            const folderBlocks = Object.fromEntries(
                Object.entries(reportJSONObj).filter(
                    ([, value]) => value.folderName === transposedData[1][j] && typeof value.isFinished !== 'undefined' && value.isFinished === true
                )
            );
        ...
        // const jsonString = JSON.stringify(reportJSONObj, null, 3);
    ...
    // const jsonString = JSON.stringify(transposedData);
    // console.log(jsonString);
    ...
    xlsx.utils.sheet_set_array_formula(worksheet, differenceTextUsingFormulaCellAddress, `=IF(${differenceFormulaCellAddress}<>"","Difference:","")`);
